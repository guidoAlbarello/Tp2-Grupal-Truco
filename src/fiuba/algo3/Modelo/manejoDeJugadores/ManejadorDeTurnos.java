...
import fiuba.algo3.Modelo.EstadosDeJuego.EstadoJuegoConFlor;
    ...
    public void pasarTurnoCartas(){
        ...
        if (this.vecesQueSePasoElTurno == this.jugadores.getTamanio()){ //SI TERMINO LA MANO
            ...
            if (this.manoActual==4 | this.jugadores.hayGanador(this.mesa.getEmparde())){
                if (this.mesa.getEmparde().getEquipo().getManosGanadas()!=3){
                    this.jugadores.getEquipoGanadorDeRonda().sumarPuntos(this.juego.getPuntosDeTruco());
                    this.reiniciarRonda();
                }else this.reiniciarRonda();
            }
        }
    }
